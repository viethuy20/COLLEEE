...
/**
 * ファンくるAPI サイト連携の OEM 側レシート・アップロード CGI 本体 (携帯用)
 *
 * 【エラーコード一覧】
 *
 ...
 *
 */
class ROI_Fancrew_SiteCooperation_ReceiptReceiveController {
	...
	public function exec($rawMail) {
		...
		if ($this->logger->isDebugEnabled()) {
			$this->logger->debug("ROI_Fancrew_ReceiptReceiveController 開始: from=$from, to = $to");
		}
...
EOF;
		...
		if ($code != 0) {
			...
			$message = <<<EOF
レシート・アップロードAPI からのエラー応答: code=$code, statusMessage=$statusMessage
■メール
from = $from
to = $to
...
EOF;
		...
		$xmlReceiptUploadResult = $xml->Data->ReceiptUploadResult;

		// レシート受付成功？
		$success = (int) $xmlReceiptUploadResult['success'];

		// ユーザに送信するメール
		$xmlEventMessage = $xmlReceiptUploadResult->EventMessage;
	...
	public function sendHttpRequest($attachments, $to, $send) {
		...
		// レシート・アップロードAPI の URL
		$url = $config->apiBaseURL . 'receipt.upload';
		...
		try {
			...
			if ($response->getStatus() != 200) {
				...
				$message = <<<EOF
レシートアップロードAPI が 200 以外の応答を返した: $status
■Request URL: $url
■Request Headers
$sRequestHeaders
