...
<section class="contents__wrap">
    ...
    <!-- page title -->
    <div class="inner">
        <div class="exchanges__ttl">
            <h3 class="contents__ttl">
                {{ Tag::image('/images/exchanges_ttl.png', 'ポイント交換') }}
            </h3>
        </div>
    </div>
    ...
    <div class="inner">
        ...
        <div class="contents__box exchanges__status">
    ...
    <!-- 交換先を選択 -->
    <div class="inner">
        <h2 class="contents__ttl u-mt-20">交換先を選択</h2>
        @php
        ...
        @endif
        <p class="exchanges__select__meta">ポイント交換の手数料については{{ Tag::link('/support/?p=258', 'こちら', ['class' => 'tohelp', 'target' => '_blank']) }}をご覧ください。</p>

        <div class="exchanges__select__tab">
            <select id="" class="" name="">
                <option value="tab_all" selected="selected">すべて</option>
                <option value="tab_bank">銀行</option>
                <option value="tab_e-money">電子マネー</option>
                <option value="tab_gift">ギフト券</option>
                <option value="tab_other">他社ポイント</option>
            </select>
        </div>
            ...
            @continue (!isset($exchange_info_map[$type]) OR ($tab_type != 'tab_all' AND $tab_type != $data['id']))
                ...
                <li>
                    <a href="{{ $exchange_info->url }}">
                        <div class="exchanges__select__list__img">{{ Tag::image('/images/'.$data['img'], $exchange_info->label) }}</div>
...
<!-- MyStyle -->
{!! Tag::script('/js/sp_exchanges.js', ['type' => 'text/javascript']) !!}
