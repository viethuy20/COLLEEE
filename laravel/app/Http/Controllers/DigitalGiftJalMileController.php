...
class DigitalGiftJalMileController extends Controller
{
    ...
    const DIGITAL_GIFT_JAL_SESSION_KEY = 'digital_gift_jal';
    const DIGITAL_GIFT_JAL_CODE = 'jalmileagebank';
    ...
    {
        // 交換情報を取得
        $exchange_info = self::checkExchangeInfo(ExchangeRequest::DIGITAL_GIFT_JALMILE_TYPE);

        $user = Auth::user();

        // セッションから取得
        $exchange = session()->get(self::DIGITAL_GIFT_JAL_SESSION_KEY, []);
        ...
        $position++;
        $link = route('jalmile.exchange', []);
        $application_json .= '{"@type": "ListItem","position": ' . $position . ', "name": "' . $exchange_info->label . '", "item": "' . $link . '"}';

        return view('digital_gift_jalmile.exchange', ['exchange_info' => $exchange_info, 'application_json' => $application_json, 'exchange' => $exchange]);
    }
    ...
    public function getConfirmView(ExchangeInfo $exchange_info, $exchange)
    {
        ...
        $application_json .= '{"@type": "ListItem","position": ' . $position . ', "name": "ポイント交換", "item": "' . $link . '"},';
        $position++;
        $link = route('jalmile.confirm');
        $application_json .= '{"@type": "ListItem","position": ' . $position . ', "name": "' . $exchange_info->label . '", "item": "' . $link . '"}';


        return view('digital_gift_jalmile.confirm', ['exchange_info' => $exchange_info, 'exchange' => $exchange, 'application_json' => $application_json]);
    }
    ...
    public function postConfirm(Request $request)
    {
        ...
        // セッションに保存
        session()->put(self::DIGITAL_GIFT_JAL_SESSION_KEY, $exchange);
    ...
    public function getConfirm()
    {
        ...
        // セッションから取得
        $exchange = session()->get(self::DIGITAL_GIFT_JAL_SESSION_KEY, null);
    ...
    public function store(Request $request)
    {
        ...
        $exchange = session()->get(self::DIGITAL_GIFT_JAL_SESSION_KEY);
        // セッション削除
        session()->forget(self::DIGITAL_GIFT_JAL_SESSION_KEY);
        ...
        $res = $this->digitalGiftService->createPurchase($exchange['point'], $purchase_code,self::DIGITAL_GIFT_JAL_CODE);
        ...
        if ($res) {
            ...
            if ($res2) {
                ...
                return redirect(route('jalmile.index') . '?logout=1')->with('message', 'ログアウトしJALマイルポイントの手続きをやり直して下さい');
            }
        } else {
            $exchange_request->rollbackRequest();
            return redirect(route('jalmile.index') . '?logout=1')->with('message', 'ログアウトしJALマイルポイントの手続きをやり直して下さい');
        ...
        // メール送信を実行
        $options = ['user' => $user, 'digital_gift_url' => $res2['gift']['url'], 'exchange_request_number' => $exchange_request->number,'code_name' => 'JALマイル'];
        ...
        $application_json .= '{"@type": "ListItem","position": ' . $position . ', "name": "ポイント交換", "item": "' . $link . '"},';
        $position++;
        $link = route('jalmile.store');
        $application_json .= '{"@type": "ListItem","position": ' . $position . ', "name": "' . $exchange_info->label . '", "item": "' . $link . '"}';

        return view('digital_gift_jalmile.store', ['exchange_request' => $exchange_request, 'application_json' => $application_json]);
    }
}
