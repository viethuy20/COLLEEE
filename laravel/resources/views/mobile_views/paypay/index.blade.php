...
<section class="major_bank">
    <section class="contents__box u-mt-20">
        <p class="img-dpoint">{!! Tag::image('images/img_paypay.png', $name.'は、さまざまなポイントへ交換が可能です') !!}</p>
        ...
        @else
            @if (empty($logout_url))
            <p class="u-mt-20">{!! Tag::link(route('paypay.oauth'), 'PayPayポイントに交換する', ['class' => 'banks__auth__btn__pink']) !!}</p>
            @else
            <p class="u-mt-20">{!! Tag::link($logout_url, 'PayPayアカウントからログアウトする', ['class' => 'banks__auth__btn__pink', 'target' => '_blank']) !!}</p>
            @endif
        @endif
    </section><!--/detailinput-->
</section>
