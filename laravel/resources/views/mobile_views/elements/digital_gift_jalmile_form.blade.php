...
<section class="selectedbank">
    ...
    @if (empty($yen_map))
    <p class="error_messsage"><span class="icon-attention"></span>交換に必要なポイントが不足しています</p>
    @else
    <style type="text/css">
    ...
    {{ Tag::formOpen(['url' => route('jalmile.confirm', ['type' => $exchange_info->type])]) }}
    @csrf    
    {{ Tag::formHidden('yens', old('yens', $exchange['yens'] ?? ''), ['id' => 'GiftCodeYen']) }}
        ...
        @endif
        {{ Tag::formSubmit('入力情報の確認', ['class' => 'banks__auth__btn__pink', 'id' => 'GiftCodeSend', 'disabled' => 'true']) }}
    {{ Tag::formClose() }}
    @endif
</section><!--/detailinput-->
