...
class TagService
{
    ...
    public function formSelect($name,$options,$selected = null,$option = [])
    {
        $option = $this->parseOption($option);
        $html = '<select name="'.$name.'" '.$option.'>';
        foreach($options as $key => $value) {
            $selectedStr = ($selected == $key) ? 'selected' : '';
            $html .= '<option value="'.$key.'" '.$selectedStr.'>'.$value.'</option>';
        }
        $html .= '</select>';
        return $this->render($html);
    }
    ...
    {
        $option = $this->parseOption($option);
        return $this->render('<input type="file" name="'.$name.'" '.$option.'>');
    }
