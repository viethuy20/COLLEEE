document.addEventListener('DOMContentLoaded', function() {
    ...
    if(address != null) {
        ...
        if(adDomain == 'gmail.com') {
            ...
            if (ua.indexOf('iPhone') > 0 || ua.indexOf('iPad') > 0) {
                mailBtn.textContent = 'メールアプリを確認する';
                mailBtn.href = 'message:';
            } else if (ua.indexOf('Android') > 0){
                mailBtn.textContent = 'メールアプリを確認する';
                mailBtn.href = 'android-app://com.google.android.gm';
            }
        }
    }
    ...
    //  *------------------------------------------*
	//  index その他SNS・メールアドレスで登録
	//  *------------------------------------------*
    const entryOpen = document.querySelector('.js-entry-open');
    if(entryOpen != null) {
        const entryHidden = document.querySelectorAll('.js-entry-hidden');

        entryHidden.forEach(function (e) {
            const eStyle = window.getComputedStyle(e, null);
            let eH = e.offsetHeight;
            const eMT = Math.ceil(eStyle.getPropertyValue('margin-top').replace(/[^0-9.]/g, ''));
            e.style.display = "none";
            entryOpen.addEventListener('click', function() {
                new Promise((resolve) => {
                    ...
                    setTimeout(() => {
                        ...
                        entryOpen.closest('li').style.display = 'none';
                    }, 10);
                }).then(() => {
                    setTimeout(() => {
	...
	//  *------------------------------------------*
	//  confirm_tel 電話かけるボタン
	//  *------------------------------------------*
    const telNumE = document.querySelector('.js-telnum');
    if(telNumE != null) {
        const telNum = telNumE.textContent;
        const telNumBtn = document.querySelector('.js-telnum-btn');
        const telNumAuth = document.querySelector('.js-telnum-auth');
        telNumE.href = 'tel:' + telNum;
        telNumBtn.insertAdjacentHTML('beforeend', telNum);
        telNumBtn.closest('a').href = 'tel:' + telNum;
        if(telNumAuth.disabled == true) {
            telNumE.addEventListener('click', function(){
                telNumAuth.disabled = false;
            });
            telNumBtn.addEventListener('click', function(){
                telNumAuth.disabled = false;
            });
        }
    }
            ...
            function check_tel_auth() {
                $.ajax({
                    ...
                    success: function(response) {console.log(response);
                        ...
                        if (check === true) {
                            const entries_auth_tel = document.getElementById('entries-auth-tel');
                            entries_auth_tel.disabled = false;
                            entries_auth_tel.click();
                        }
